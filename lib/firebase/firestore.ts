...
export async function createDepositRequest(
  ...
  await createUserNotification(uid, {
    ...
    message: `Your ${asset.symbol} payment notice for ${expectedAmount} USD is awaiting confirmation from NovaVest.`,
    type: 'info',
...
export async function createInvestmentRequest(
  ...
  await createUserNotification(uid, {
    ...
    message: `${payload.planName} is now awaiting confirmation from NovaVest.`,
    type: 'info',
...
export async function createWithdrawalRequest(
  ...
  await createUserNotification(uid, {
    ...
    message: `Your withdrawal request for ${amount} USD is awaiting confirmation from NovaVest.`,
    type: 'info',
...
export async function createLoanRequest(uid: string, amount: number, termMonths: number, purpose: string) {
  ...
  await createUserNotification(uid, {
    ...
    message: `Your loan request for ${amount} USD is awaiting confirmation from NovaVest.`,
    type: 'info',
    link: '/dashboard/loan',
  })
}
...
export async function setDepositRequestStatus(requestId: string, status: RequestStatus, adminUid: string, adminNote: string) {
  ...
  if (status === 'approved') {
    await createTransactionRecord({
      ...
      description: `${request.assetSymbol} deposit confirmed by NovaVest`,
...
export async function setInvestmentRequestStatus(requestId: string, status: RequestStatus, adminUid: string, reviewerNote: string) {
  ...
  if (status === 'approved') {
    await createTransactionRecord({
      ...
      description: `${request.planName} package activated by NovaVest`,
    })
  }
...
export async function setWithdrawalRequestStatus(requestId: string, status: RequestStatus, adminUid: string, adminNote: string) {
  ...
  await createUserNotification(request.userId, {
    ...
    message: status === 'approved'
      ? `Your withdrawal request for ${request.amount} USD has been confirmed by NovaVest.`
...
export async function adjustUserBalance(userId: string, amount: number, note: string) {
  ...
  await createUserNotification(userId, {
    ...
    message: note || `NovaVest ${amount >= 0 ? 'credited' : 'debited'} your account by ${Math.abs(amount)} USD.`,
...
export async function setLoanRequestStatus(requestId: string, status: RequestStatus, adminUid: string, adminNote: string) {
  ...
  await createUserNotification(request.userId, {
    ...
    message: status === 'approved'
      ? `Your loan request for ${request.amount} USD has been confirmed by NovaVest.`
  ...
  const coreAssets = [
    {
      ...
      note: 'Send BTC to this wallet only. After sending, tap "I\'ve Made the Payment" so NovaVest can confirm it.',
    },
    {
      symbol: 'ETH',
      name: 'Ethereum',
      ...
      note: 'Send ETH to this wallet only. After sending, tap "I\'ve Made the Payment" so NovaVest can confirm it.',
    },
  ] as const
